...
- Installed Ktor 3.5.2 omits `sin6_addr` in its native IPv6 sockaddr packing. The iOS I/O adapter
  ...
  are recorded in [the verification report](../development/torrent-verification.md). JVM/Android use Ktor sockets.
...
- Ownership records include OS file identity and are appended independently of TaskStore snapshots.
  ...
  Descriptor-rooted parent traversal and no-follow payload opens now reject concurrent symlink
  redirection. Final-entry identity checking and deletion remain separate OS operations; the

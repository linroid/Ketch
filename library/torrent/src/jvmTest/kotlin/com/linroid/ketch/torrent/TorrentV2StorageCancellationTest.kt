...
class TorrentV2StorageCancellationTest {
  @Test
  fun blockedWriteOwnsItsBufferAndPublishesNothingAfterCancellation() = runTest {
    ...
    val root = FileSystem.SYSTEM_TEMPORARY_DIRECTORY /
      "ketch-v2-blocked-${InfoHash.fromBytes(torrentRandomBytes(20)).hex}"
    ...
    val provider = object : ForwardingFileSystem(torrentFileSystem) {
      override fun openReadWrite(file: Path, mustCreate: Boolean, mustExist: Boolean): FileHandle {
        if (blockWrites && file.name == "payload") {
